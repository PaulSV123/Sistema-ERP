...
const DeleteModal = ({isOpen,close,data,actualizar}) => {
    ...
    const eliminarRecurso =(e)=>{
       e.preventDefault();
      deleteEliminarDatosIntegracion(data.id).then((message)=>{
        actualizar()

        message ? alert("Dato Eliminado con Éxito!!") :alert("Error al  Eliminar!!")
      }
      )
      close();
    }
