...
const GraficoCircular_Dia = ({xsemana, nota, datos}) => {

  // get dates
  const semanaTitle = xsemana;
  const valorDatosSemana = nota || [];

  // get values for week
  const labelSemana = Object.keys(valorDatosSemana);
  const valorSemana = Object.values(valorDatosSemana);
  ...
  return (
    <div>
      <button onClick={handleSubmit}>DatosPrueba Dias</button>
      <br />
...
export default GraficoCircular_Dia;
